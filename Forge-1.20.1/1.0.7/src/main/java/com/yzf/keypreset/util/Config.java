...
public class Config {
    ...
    // ============= 获取键位显示名 =============
    public static Component getKeyBindingDisplayName(int index) {
        try {
            ...
            for (String part : parts) {
                InputConstants.Key key = InputConstants.getKey(part);
                // 转成翻译后的显示名
                Component nameComp = key.getDisplayName();
                displayNames.add(nameComp.getString());
            }

            return Component.literal(String.join(" + ", displayNames));
        } catch (Exception e) {

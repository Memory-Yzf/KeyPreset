...
public class KeyPresetManager {
    ...
    public static void savePreset(String name) {
        ...
        try (FileWriter writer = new FileWriter(KeyPreset.PRESET_FOLDER.resolve(name + ".json").toString())) {
            ...
            KeyPreset.LOGGER.error("保存预设文件失败: {}", e.getMessage());
        }
    }
    ...
    public static void deletePreset(String name) {
        ...
        try {
            ...
            KeyPreset.LOGGER.error("删除预设文件失败: {}", e.getMessage());
        }
    }
    ...
    public static void loadPreset(String name) {
        ...
        try (FileReader reader = new FileReader(KeyPreset.PRESET_FOLDER.resolve(name + ".json").toFile())) {
            Type linkedHashMapType = new TypeToken<LinkedHashMap<String, String>>() {
            }.getType(); // 创建泛型类型标识（解决类型擦除问题）
                ...
                if (keyBindsMap.containsKey(key)) {
                    ...
                    if (parts.length > 1) {
                        try {
                            ...
                            KeyPreset.LOGGER.warn("未知修饰键: {}", parts[1]);
                        }
                    }
            ...
            KeyPreset.LOGGER.error("加载预设文件失败: {}", e.getMessage());
        }
    }
    ...
    public static void openPresetFolder() {
        ...
        try {
            ...
            KeyPreset.LOGGER.error("打开预设文件夹失败: {}", e.getMessage());
        }
    }
    ...
    public static String[] getPresets() {
        ...
        try (Stream<Path> stream = Files.list(KeyPreset.PRESET_FOLDER)) {
            return stream
                    ...
                    .filter(p -> {
                        try (FileReader reader = new FileReader(p.toFile())) {
                            ...
                            LinkedHashMap<String, String> data = gson.fromJson(new JsonReader(reader), mapType);
                            return data != null && !data.isEmpty();
                        } catch (Exception e) {
                            KeyPreset.LOGGER.warn("非法预设文件：{}，原因：{}", p.getFileName(), e.toString());
                            return false;
                        }
                    })
                    // 文件名转换处理
            ...
            KeyPreset.LOGGER.error("获取预设文件失败: {}", e.getMessage());
    ...
    public static void createPresetFolder() {
        try {
            ...
            KeyPreset.LOGGER.error("创建预设文件夹失败: {}", e.getMessage());
        }
    }
}